...
请编写一个函数`solution`，接收一个数组arr，并返回它的前缀和，对于数组`a1, a2, a3...an`，它的前缀和是`s1, s2, s3...sn`，其中`sx = a1 + a2 + a3 ... ax`
...
请编写一个函数`solution`，接收一个数组arr，以及一个可选的数字x，将传入的数组向右旋转x次（如果不传入x，则向右旋转1次），然后将其返回。其中，向右旋转指的是，将每个元素向后移动一次，如果移动的元素是该数组的最后一个元素就移动到开头
