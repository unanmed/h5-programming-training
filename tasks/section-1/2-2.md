...
请编写一个函数`solution`，它可以接受一个参数`compare`作为比较器，该参数是函数，然后接受一系列参数，让该参数与其他参数一一比较是否相等，最后返回这些参数中不相等的数量（在本题中，如果两个参数`a`，`b`有`compare(a, b) === true`，则认为他们是相等的，否则不相等）。传入的值从`arguments[1]`开始依次向后

保证传入的比较函数满足交换律，传递性和反传递性，即若`a === b`则`b === a`, 若`a === b`且`a === c`则`b === c`
