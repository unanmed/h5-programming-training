...
提示：不需要判断包含敏感词的情况，只需要判断是否相等，比如如果敏感词为"NB",发言为["h5NB"]不需要和谐。

## 样例：

输入
```js
comments = [ [ "LHJNB" ], [ "太难了" ], [ "太难了", "太难了", "太难了", "太难了", "太难了", "能不能简单点" ], [ "太难了", "能不能简单点" ] ] // 发言列表
censorlist = [ "太难了" ] // 敏感词列表
word = "H5NB" // 敏感词被换为"H5NB"
```

输出
```js
// 第一条发言为[ "LHJNB" ] 其中不包含任何敏感词，直接返回
// 对于第二条发言 ${ code(comments[1]) } 只包含敏感词 从列表中删除
// 第三条发言 敏感词超过5次 删除
// 第四条发言 包含一个敏感词和一个非敏感词 对敏感词和谐
[["LHJNB"],["H5NB","能不能简单点"]]
```
...
const censorlist = [ "太难了" ];
const word = "H5NB";
const example = `
发言列表 = ${ code(comments) }
敏感词列表 = ${ code(censorlist) }
和谐词 = ${ code(word) }
对于第一条发言 ${ code(comments[0]) } 其中不包含任何敏感词，因此可以直接返回
对于第二条发言 ${ code(comments[1]) } 其中包含一个敏感词 ${ code(comments[1][0]) } ，因此需要替换为 ${ code(word) }
对于第三条发言 ${ code(comments[2]) } 其中出现了五次敏感词，因此需要被和谐
因此最终的返回应该是 ${ code([ comments[0], [ word ] ]) }
`;
return [
    [ comments, censorlist, word ], example
];
```
