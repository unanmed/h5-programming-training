...
name为画布名，也可为画布的context2d对象，即`createCanvas`的返回值，style是矩形的颜色，如果angel不为空，将会将矩形旋转对应的弧度(一般是Math.PI*某个值)
...
在id为name的画布的对应位置上绘制一段文字 style为文字的样式 font是文字字体
...
你不仅可以把图片绘制到你的画布上，也可以把另一个画布的内容或者当前视频的某一帧当做图片画到你的画布上，样板的浏览地图`core.maps.drawThumbnail`正是使用了这个办法，把新楼层的各个图层画布都画到另一个画布上，达成了绘制楼层缩略图的效果。
